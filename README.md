
<p align="center">
  <img src="assets/logo-dynamic.svg" width="350">
</p>

<h1 align="center">Welcome to my Data Science Portfolio</h1>

<h1 align="center"> Malek Senoussi – Data Scientist & ML Engineer</h1>

<p align="center">
PhD in Applied Mathematics · Machine Learning · AI Engineering · Scientific Computing  
</p>

---

# 📁 Portfolio Overview

Welcome to the repository hosting my **Data Science & Machine Learning portfolio**, built with GitHub Pages.

It includes:
- My CV  
- AI/ML projects  
- Dashboards & notebooks  
- Research pipelines  
- Interactive visualizations  

👉 **Live website:** [Portfolio](https://maleksnous.github.io/)

---

# 🧠 Featured Projects



### **1. Single-Cell RNA-seq ML Pipeline**
- PyTorch, Scanpy, scVI  
- Graph models & high-dimensional clustering  
- Dash dashboard  

### **2. Graph network and random walk**
- Analyse graph and network
- Random walk
- Dash interactive UI  


---

# 🚀 Tech Stack

- **Languages:** Python, R, SQL  
- **ML:** PyTorch, scikit-learn, TensorFlow  
- **Data:** Pandas, Polars, Scanpy  
- **Visualisation:** Dash, Plotly, Seaborn, matplotlib
- **MLOps:** GitHub Actions, Docker, Azure ML  

---

# 📫 Contact

- Email: malek.senoussi@gmail.com
- GitHub: [Malek Snous](https://github.com/MalekSnous)
- LinkedIn: [Malek Sen](https://www.linkedin.com/in/maleksen)

---

<p align="center">© 2025 – Designed & Built by Malek Senoussi</p>


